...
class ServeyController extends Controller
{
    public function index(Request $request){
        ...
        $questionCount = Question::where('id',$request->input('question_id'))->count();
       ...
       if($question->ads_id >= 1){
        ...
        if($answerSubmittedByUser >= 1){
            ...
            if($questionCount > 0){
                ...
                $hasVisitedCount = UserServey::where('question_id', $request->input('question_id'))->where('user_id', Auth::user()->id)->count();
    ...
    public function singleServeyAnswer(Request $request){
        ...
        $hasSubmittedThisAnsCount = UserAnswer::where('question_id', $request->get('question_id'))->where('user_id', Auth::user()->id)->count();
        if($request->has('answer_id')){
            if($hasSubmittedThisAnsCount >= 1){
    ...
    public function getInstantResult($question_id){
        ...
        if($hasServeySubmittedUserCout >= 1){
            return view('game.serveys.servey-result', compact('colorArrays', 'answered', 'skipped', 'serveyResults', 'otherAnswersCount', 'questionDetails', 'overFlowCount')); 
        }else{
